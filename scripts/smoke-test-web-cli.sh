...
#   ├── static/              ← SPA assets
#   └── bundled-chislcore/<plat-arch>/...
for dir in static bundled-chislcore; do
  if [ ! -d "$dir" ]; then
...
BACKEND_DIR="bundled-chislcore/$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/aarch64/arm64/; s/x86_64/x64/')"
BACKEND_BINARY="$BACKEND_DIR/chislcore"
if [ ! -x "$BACKEND_BINARY" ]; then
  echo "❌ Backend binary missing or not executable: $BACKEND_BINARY"
  exit 1
fi
# chislcore has no --version flag. Read the pinned version from manifest.json
# (which prepareChislcore writes at pack time) and use --help to confirm the
