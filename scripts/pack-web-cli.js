...
// 1. Prepare bundled-chislcore
console.log('1. Preparing chislcore...');
prepareChislcore({
  projectRoot,
  platform,
  arch,
  version: resolveChislcoreVersion(projectRoot),
});
...
// 7. Copy bundled-chislcore
const backendSrc = path.join(projectRoot, 'resources/bundled-chislcore', `${platform}-${arch}`);
const backendDest = path.join(tarballContentDir, 'bundled-chislcore', `${platform}-${arch}`);
if (!fs.existsSync(backendSrc)) {
  throw new Error(`Backend bundle dir missing at ${backendSrc}. Ensure prepareChislcore succeeded.`);
}
