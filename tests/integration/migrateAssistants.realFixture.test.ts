 ...
 * chislcore binary using the user-provided fixtures
 ...
 *   3. Spawn a real chislcore bound to a throw-away data-dir.
...
function resolveBackendBinary(): string {
  const candidates = [
    ...
    path.join(os.homedir(), '.cargo', 'bin', 'chislcore'),
    path.resolve(__dirname, '../../../AionCore/target/debug/chislcore'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('chislcore binary not found (set AIONUI_BACKEND_BINARY or build it)');
}
