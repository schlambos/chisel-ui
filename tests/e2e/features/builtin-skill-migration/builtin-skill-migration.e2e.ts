...
function resolveBackendBinary(): string {
  const candidates = [process.env.AIONUI_BACKEND_BINARY, path.join(os.homedir(), '.cargo', 'bin', 'chislcore')].filter(
    (x): x is string => typeof x === 'string' && x.length > 0
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error('chislcore binary not found. Set AIONUI_BACKEND_BINARY or install to ~/.cargo/bin/chislcore.');
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  //
  // Run against a sibling `chislcore` process on port 25903 against a
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      const logPath = path.join(dataDir, 'sibling-chislcore.log');
