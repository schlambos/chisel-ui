import { startWebHost, startStaticServer } from '@chisl/web-host';
import type { WebHostHandle, StaticServerHandle } from '@chisl/web-host';
...
//   ├── bundled-chislcore/<plat-arch>/chislcore[.exe]
//   └── static/                  ← SPA assets
//
...
const BACKEND_BINARY = process.platform === 'win32' ? 'chislcore.exe' : 'chislcore';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const bundled = path.join(cliRoot, 'bundled-chislcore', platArch, BACKEND_BINARY);
  return bundled;
}
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download chislcore and set AIONUI_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('Chisel WebUI (frontend only) is ready');
    ...
    console.log('');
    console.log('Chisel WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
